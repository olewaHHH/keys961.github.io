...
所以，这里生成另一个**事件序列$seq'$**，它是$seq$的一个排列，且**满足pre-recording event一定发生在post-recording event之前**（如上面的例子，可能有$e'_{j-1} = e_j, e'_{j} = e_{j-1}$）。**下面会证明：$S^*$就是在所有pre-recording event之后，post-ordering event之前的全局状态**。
